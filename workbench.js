// workbench.js (shell-compatible: uses window.ABM.sb, does NOT create client)
(() => {
  ...
  function initOnce() {
    ...
    // Hard guards: shell MUST exist
    if (!window.ABM?.sb) {
      console.error("[Workbench] ABM shell missing. window.ABM.sb not found.");
      alert("ABM App Shell not loaded. window.ABM.sb is missing.");
      return;
    }

    const sb = window.ABM.sb;

    // Ensure we do NOT overwrite window.ABM (shell owns it)
    window.ABM.me = window.ABM.me || null;
    window.ABM.currentRole = window.ABM.currentRole || null;
    ...
    function applyDialCodeToInput(inputEl, dialCode) {
      ...
      if (!v) { inputEl.value = dialCode + " "; return; }
      if (/^\+\d+/.test(v)) { inputEl.value = v.replace(/^\+\d+/, dialCode); return; }
    ...
    function mapViewToWorkbenchView(view) {
      if (view === "done" || view === "rejected") return "v_workbench_queue_done";
    ...
    function renderCampaignDropdownForClient(clientId) {
      ...
      const rows = cid
        ? (campaignOptionsCache || []).filter(r => (r.client_id || "").toString() === cid)
      ...
      campaignSel.innerHTML =
        ...
        rows
          ...
          .map(r => `<option value="${r.campaign_id}">${r.campaign_name}</option>`)
          .join("");
    }
    ...
    async function loadClientCampaignOptions() {
      ...
      clientSel.innerHTML =
        ...
        [...clients.entries()].map(([id, name]) => `<option value="${id}">${name}</option>`).join("");
    ...
    async function loadQueue() {
      ...
      const view = $("viewSelect")?.value || "pending_in_progress";
      ...
      if (viewName === "v_workbench_queue") q = q.in("enrichment_status", statuses);
      ...
      if (viewName === "v_workbench_queue_done") {
        if (view === "done") queueRows = queueRows.filter(r => (r.enrichment_status || "").toLowerCase() === "done");
        if (view === "rejected") queueRows = queueRows.filter(r => (r.enrichment_status || "").toLowerCase() === "rejected");
      }
    ...
    function renderQueue() {
      ...
      const rows = (queueRows || []).filter(r => {
        ...
        const hay = [r.first_name, r.last_name, r.email, r.company, r.title, r.enrichment_status]
        ...
        return `
          <tr class="${selected}" data-key="${key}">
            ...
            <td style="text-align:right;">
              <div class="actions">
                <button class="btn-ghost" data-action="open" type="button">Open</button>
                <button class="btn" data-action="claim" type="button">Claim</button>
              </div>
            </td>
          </tr>
        `;
    ...
    async function openLead(ingest_job_id, row_number) {
      ...
      try {
        ...
        if ($("linkedinUrl")) $("linkedinUrl").value = (currentFacts?.linkedin_profile_url || "");
        if ($("location")) $("location").value = (currentFacts?.person_location || "");
    ...
    async function saveLead() {
      ...
      try {
        ...
        if (!currentLeadId) throw new Error("Cannot save lead_facts: lead_id not found yet.");
        ...
        const { error: factsErr } = await sb
          .from("lead_facts")
          .upsert(
            { lead_id: currentLeadId, linkedin_profile_url, person_location, seniority, department, company_industry, linkedin_company_url },
            { onConflict: "lead_id" }
          );
    ...
    async function releaseLead() {
      ...
      try {
        const { error } = await sb
          .from("stg_leads")
          .update({ enrichment_status: "pending", enriched_by: null, enriched_at: null })
          .eq("ingest_job_id", ingest_job_id)
          .eq("row_number", row_number);
    ...
    async function markRejected() {
      ...
      try {
        ...
        const { error } = await sb
          .from("stg_leads")
          .update({ enrichment_status: "rejected", enrichment_notes: notes, enriched_at: new Date().toISOString() })
          .eq("ingest_job_id", ingest_job_id)
          .eq("row_number", row_number);
    ...
    $("clearBtn")?.addEventListener("click", async () => {
      ...
      renderCampaignDropdownForClient("");
      await loadQueue();
    });
